...
//! Functional integration test for a policy whose keys come from a live
//! JWKS endpoint.
//!
//! The other policy tests use inline secrets and fetch nothing, so none
...
/// The signing secret, and the `k` value the JWKS publishes for it.
///
/// A symmetric key keeps the test to one shared value: the JWKS carries
/// the same bytes the token was signed with, so a successful verification
/// can only mean the document was fetched.
...
/// A local JWKS server on an OS thread, so it is serving before the proxy
/// starts and outlives every runtime the proxy builds.
struct JwksEndpoint {
    /// Where the policy's `jwks_url` should point.
    address: SocketAddr,

    /// How many times the document was served.
    fetches: Arc<AtomicUsize>,
}

impl JwksEndpoint {
    /// Start serving a one-key JWKS document.
    fn start() -> Self {
        let document =
...
/// Mint an HS256 JWT whose `kid` names the key the JWKS publishes.
...
/// Write a policy document whose only identity plugin resolves its keys
/// from `jwks_url`.
...
/// Load the policy example, point it at the JWKS-backed policy document,
/// and permit the loopback identity provider the test runs.
...
fn a_token_signed_by_an_unpublished_key_is_rejected() {
    // The control: without it, a plugin that silently accepted every
    // token would pass the test above whether or not the fetch happened.
    let jwks = JwksEndpoint::start();
    let backend_guard = start_backend_with_shutdown("ok");
    let policy_dir = tempfile::TempDir::new().expect("create tempdir");
    let policy_path = write_jwks_policy(&policy_dir, jwks.address);
